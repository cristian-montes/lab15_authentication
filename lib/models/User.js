...
module.exports = class User {
  ...
  //--------------------------------------------------------------------------------//
  static async upgradesCustomerById(id, objBody){

    const role =  await Role.findByTitle(objBody.role);

    const { rows } = await pool.query(
      'UPDATE users SET email=$2, role_id=$3 WHERE id=$1 RETURNING *',
      [id, objBody.email, role.id] 
    );
    return  new User({ ...rows[0], role: role.role_title });
  }
